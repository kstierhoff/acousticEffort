---
title: "Acoustic Line Kilometers-CalCOFI (Spring 2021)"
...
# Source survey info ------------------------------------------------------
source(here("Code/settings_2101RL_CalCOFI.R"))
...
if (get.nav) {
  nav <- read_csv(here("Data/SCS/2101RL/SAMOS-OBS_fixed.elg")) %>% 
    mutate(time = mdy_hms(paste(Date, Time))) %>% 
    select(time, 
           lat = `SAMOS-Lat-VALUE`, 
           long = `SAMOS-Lon-VALUE`,
           SST = `SAMOS-TSGT-VALUE`,
           SOG = `SAMOS-SOG-VALUE`) %>% 
  ...
  if (sd.survey) {
    ...
    nav <- nav %>% 
      mutate(
        SOG      = SOG/1.94384, # Convert m/s to knots
        date     = date(time),
        SST      = na_if(SST, NaN),
        SOG      = na_if(SOG, NaN),
        # date = date(time),
        dist = SOG*0.51444*60/1000) %>%  # Distance in km, from knots
      filter(between(long, min.long, max.long), 
             between(lat, min.lat, max.lat))
...
# Summarise nav
mean.sog <- mean(nav$SOG, na.rm = TRUE)
```
...
if (sd.survey) {
  ...
  nav.depth <- get.depth(bathy, nav$long, nav$lat, locator = F, distance = T) %>% 
    ...
    filter(dist.depth < 100) %>% 
    mutate(depth_bin = cut(depth, c(min(depth), -200, 0), include.lowest = T, labels = F),
           id = cut(as.numeric(time), as.numeric(nav.daynight$time), include.lowest = T, labels = F),
           depth_bin = case_when(
             depth_bin == 1 ~ ">200m",
             depth_bin == 2 ~ "< 200 m")) %>% 
  ...
  nav.summ <- nav.depth %>% 
    ...
    summarise(dist_km = round(sum(dist, na.rm = TRUE))) %>% 
